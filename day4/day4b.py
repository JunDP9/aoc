...
def prepare_array(board_size, input_list):
    ...
    for idx in range(0, len(input_list)):
        ...
        if 'board ' + str(board_counter) not in boards_amt_bingo:
            boards_amt_bingo['board ' + str(board_counter)] = 0
        if idx % board_size == 0:
            board_counter += 1
        for number in nums:
...
def check_bingo(board):
    board_size = 5
    for idx in range(0, board_size):
        row = board[idx * board_size:board_size + idx * board_size]
        col = []
        for kdx in range(0, board_size):
            col.append(board[idx + (kdx * board_size)])
        ...
        for key, value in boards_array.items():
            ...
            for number in value:
                if int(bingo_numbers[idx]) == int(number[0]):
                    number[1] = 1
                    if check_bingo(value):
                        boards_amt_bingo.pop(key, None)
                        if len(boards_amt_bingo) == 1:
                            print(key)
